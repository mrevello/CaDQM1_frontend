...
export const SchemaVisualizer: React.FC<SchemaVisualizerProps> = ({
  ...
  const { initialNodes, initialEdges } = useMemo(() => {
    const leftModels = models.filter(m => m.fields.some(f => f.hasOutgoing));
    const rightModels = models.filter(m => !m.fields.some(f => f.hasOutgoing));
  ...
  }, [initialNodes, selectedTable, setNodes]);
  ...
  return (
    <Box height="60vh" boxSizing="border-box" border="1px solid #e5e7eb" borderRadius={2} flex={1}>
      <ReactFlow
        nodes={nodes}
        edges={edges}
